...
"""
    ...
    def generate_index_page(self):
        """Generate/update index.md with navigation"""
...
"""
        ...
        index_content += "\n### Guides\n"
        guide_pages = [p for p in all_pages.keys() if p.startswith('guides/')]
        for page in sorted(guide_pages):
            title = Path(page).stem.replace('-', ' ').title()
            index_content += f"- [{title}]({page})\n"
        
        index_content += "\n### Changelog\n"
        changelog_pages = [p for p in all_pages.keys() if p.startswith('changelog/')]
        for page in sorted(changelog_pages):
            title = Path(page).stem.replace('-', ' ').title()
            index_content += f"- [{title}]({page})\n"
